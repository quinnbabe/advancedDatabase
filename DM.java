...
public class DM {
	...
	public final static int locksize=50;
	TM TM;
	HashMap<Integer, Transaction> transactionList;
	ArrayList<Site> siteList;
	public int [][] sLockTable=new int[11][21];//sLockTable[siteId][dataId] 0-no lock 1-readLock -2-writelock 
    public int [][][] tranId=new int [11][21][locksize];//tranId[siteId][dataId]=transactionId holding this data
    ...
    public int alreadyHasWriteLock(int transactionId, int dataId){
    	int siteNo=-1;
    	...
    	for(int i=0;i<sites.size();i++){
    		...
    		if(site.siteStatus==SiteStatus.AVAILABLE
    		  &&sLockTable[siteId][dataId]==-2&&tranId[siteId][dataId][0]==transactionId)
    		   siteNo=siteId;
    	}
    	return siteNo;
    }
    public void readFromLog(int transactionId,int dataId,int siteId){
    	 if(siteList.get(siteId-1).log[dataId]==-1){
    		 System.out.println("Log is empty");
    	 }
    	 else
    	  System.out.println("read from log successful,"
    	  		+ " value is"+ siteList.get(siteId-1).log[dataId]);
    	  }
	public int Read(int transactionId, int dataId) {
		...
		if (transactionList.get(transactionId).transactionType == TransactionType.READONLY) {
			return TM.ReadOnly(transactionId, dataId);
               ...
               //0. if has write lock on data
			   if(alreadyHasWriteLock(transactionId, dataId)!=-1){
				   int siteId=alreadyHasWriteLock(transactionId, dataId);
				   readFromLog(transactionId,dataId,siteId);
			   }
			   // 1. This transaction has read lock on this data
			   else if (alreadyHasReadLock(transactionId, dataId)!=-1) {
				int siteId = alreadyHasReadLock(transactionId, dataId);
			...
			// 2.This transaction has no read lock on this data
				...
				// 3.2  No write lock,can get read lock
					...
					int readSite = obtainReadLock(transactionId, dataId);
					if (readSite > 0 && readSite <= 10) {
						readFromDataList(transactionId, dataId, readSite);
						//PrintLockTable();
						return 0;
					} else {
						System.out.print("ERROR:tmpsite is not in 1-10,obtain read lock fail.");
						return -1;
					}
				}
			}
		...
		if (transactionList.get(transactionId).transactionType == TransactionType.READWRITE) {
			if (transactionList.get(transactionId).transactionStatus == TransactionStatus.RUNNING) {
				...
				// 2. Need to require lock on all sites
					...
					for(int i=0;i<sites.size();i++){
						executeLock(transactionId, dataId, sites.get(i).siteId,
								LockType.WRITE);
					}
					writeOnLog(transactionId, dataId, sites, val);
					// printX2();
				...
				System.out.print("Transaction "+transactionId+" is not running.\n");
			...
			System.out.println("Read Only can not write");
			System.exit(-1);
...
//For TM
	...
	//New change
//	public ArrayList<Integer> getRunningTransactionIds() {
...
//	}
	public boolean checkAvailability(int dataId, int siteId) {
		return (siteList.get(siteId - 1).siteStatus == SiteStatus.AVAILABLE
				&& !siteList.get(siteId - 1).replicatedData.contains(dataId));
...
//			  }
	...
	public boolean siteOtherTranHoldData(int siteId,int transactionId,int dataId){
		    
			if (sLockTable[siteId][dataId]==-2&&tranId[siteId][dataId][0]!=transactionId)
				return true;
			for(int i=0;i<locksize;i++){
			      if(sLockTable[siteId][dataId]>0&&tranId[siteId][dataId][i]!=transactionId)
			    	  return true;
			}
	...
	//New change
	public boolean tranNotAbort(int trId){
		return (transactionList.get(trId).transactionStatus!=TransactionStatus.ABORTED);
//		ArrayList<Integer> runningTransactions = getRunningTransactionIds();
//		return runningTransactions.contains(trId);
    ...
    public boolean otherTranHoldWriteLockOnData(int transactionId, int dataId){
    	...
    	for(int i=0;i<sites.size();i++){
    		...
    		if(trId!=-1&&sLockTable[siteId][dataId]==-2
    		  &&tranNotAbort(trId)&&trId!=transactionId)
    			return true;
    	}
    	return false;

    }
	public int obtainReadLock(int transactionId, int dataId) {
		int tmpSiteId = -1;
		ArrayList<Site> sites = getSitesWithData(dataId);
		for (int i = 0; i < sites.size(); i++) {
	...
	public boolean checkWriteLockWithAllSites(int transactionId, int dataId) {
		...
		Transaction tran=transactionList.get(transactionId);
		for (int i = 0; i < sitesWithData.size(); i++) {
			if (sitesWithData.get(i).siteStatus == SiteStatus.AVAILABLE
			&&(!tran.hasThisWriteLock(sitesWithData.get(i), dataId))) 
	...
	public void executeLock(int transactionId, int dataId, int siteId,
		...
		if(type==LockType.READ){
			if(sLockTable[siteId][dataId]==locksize){
				System.out.print("LockSize ERROR");
			}
	...
	public int alreadyHasReadLock(int transactionId, int dataId) {
		ArrayList<Site> sites = getSitesWithData(dataId);
		Transaction tran=transactionList.get(transactionId);
		for (int i = 0; i < sites.size(); i++) {
			if (sites.get(i).siteStatus == SiteStatus.AVAILABLE) {
				if (tran.hasThisReadLock(sites.get(i).siteId, dataId))
	...
	public void readFromDataList(int transactionId, int dataId, int siteId) {
		...
		System.out.println("R(T" + transactionId + ",x" + dataId
				+ ") successful , value is "
				+ siteList.get(siteId - 1).ReadDataList(dataId)+" from site "+ siteId);
	...
	public boolean hasWriteLock(int tid, int varid) {
		...
		for (int i = 0; i < sites.size(); i++) {
			if (sites.get(i).siteStatus == SiteStatus.AVAILABLE
			   &&transactionList.get(tid).hasThisWriteLock(sites.get(i),varid)) 
